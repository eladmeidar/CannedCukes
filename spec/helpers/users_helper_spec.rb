...
describe UsersHelper do
  ...
  describe "link_to_user" do
    ...
    it "should use the login as link text with no :content_method specified" do
      link_to_user(@user).should have_tag("a", @user.login)
    end
    it "should use the name as link text with :content_method => :name" do
      link_to_user(@user, :content_method => :name).should have_tag("a", @user.name)
    end
    ...
    it "should use the name as link title with :content_method => :name" do
      link_to_user(@user, :title_method => :name).should have_tag("a[title='#{@user.name}']")
    end
  ...
  describe "link_to_current_user, When logged in" do
    ...
    it "should use the name as link text with :content_method => :name" do
      link_to_current_user(:content_method => :name).should have_tag("a", @user.name)
    end
    it "should use the login as title with no :title_method specified" do
      link_to_current_user().should have_tag("a[title='#{@user.login}']")
    end
    it "should use the name as link title with :content_method => :name" do
      link_to_current_user(:title_method => :name).should have_tag("a[title='#{@user.name}']")
    end
